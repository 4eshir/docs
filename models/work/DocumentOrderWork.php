...
class DocumentOrderWork extends DocumentOrder
{
    ...
    public function rules()
    {
        return [
            ...
            [['order_date', 'allResp', 'groups_check', 'participants_check', 'new_groups_check'], 'safe'],
            [['state'], 'boolean'],
    ...
    public function afterSave($insert, $changedAttributes)
    {
        ...
        $status = 2 ; // 0 - зачислен, 1 - отчислен, 2 - перевод
//var_dump( $this->groups_check === null);
        ...
        if (true)   // тут было условие которое ловило баг, теперь всё работает как надо
        {
            ...
            foreach ($delGroups as $delGroup)
            {
                ...
                {
                    /**/ // удаление из связки
                    $ordersGroupParticipant = OrderGroupParticipantWork::find()->where(['order_group_id' => $order->id])->all();
    ...
    public function beforeDelete()
    {
        ...
        {
            /**/ // удаление из связки
            $ordersGroupParticipant = OrderGroupParticipantWork::find()->where(['order_group_id' => $group->id])->all();
            foreach ($ordersGroupParticipant as $del)
                $del->delete();
            /**/
            $group->delete();
