...
class CertificatTemplatesWork extends CertificatTemplates
{
    ...
    {
        /*$certificats = CertificatWork::find()->where(['certicicat_template_id' => $this->id])->all();

        if (count($certificats) > 0)
        {
            Yii::$app->session->setFlash('danger', 'Невозможно удалить шаблон, т.к. имеются связанные с ней сертификаты!');
            return;
        }*/
