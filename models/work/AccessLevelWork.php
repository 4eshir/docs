...
class AccessLevelWork extends AccessLevel
{
    ...
    public function afterSave($insert, $changedAttributes)
    {
        ...
        $sqlStr = "CREATE EVENT `token_event".$this->id."` ON SCHEDULE AT '".date('Y-m-d H:m:s', strtotime($date->format('Y-m-d H:m:s')))."' ON COMPLETION NOT PRESERVE ENABLE DO DELETE FROM `access_level` WHERE `id`=".$this->id;
        Yii::$app->db->createCommand($sqlStr)->execute();
    }

    public function beforeSave($insert)
    {
        $this->start_time = date('Y-m-d H:m:s');
        ...
        $this->end_time = date('Y-m-d H:m:s', strtotime($date->format('Y-m-d H:m:s')));
