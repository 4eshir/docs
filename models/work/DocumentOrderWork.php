...
class DocumentOrderWork extends DocumentOrder
{
    ...
    {
        //$parts = OrderGroupParticipantWork::find()->joinWith(['orderGroup orderGroup'])->where(['orderGroup.document_order_id' => $this->id])->all();
        //$participantName = ForeignEventParticipantsWork::find();
        //$part->where(['id' => $groupParticipant->participant_id])->one()->getFullName();
        $result = '';
    ...
    public function afterSave($insert, $changedAttributes)
    {
        ...
        $status = 0 ; // 1 - зачислен, 2 - отчислен, 3 - переведен, 0 - неизвестное значение
//var_dump( $this->groups_check === null);
        ...
        if (true)   // тут было условие которое ловило баг, теперь всё работает как надо
        {
            ...
            foreach ($delGroups as $delGroup)
            {
                ...
                {
                    /**/ // удаление из связки
                    $ordersGroupParticipant = OrderGroupParticipantWork::find()->where(['order_group_id' => $order->id])->andWhere(['status' => $status])->all();
                    foreach ($ordersGroupParticipant as $del)
                    {
                        if ($del !== null)
                            $del->delete();
                    }
        ...
        if ($this->participants_check !== null && count($this->participants_check) > 0)
        {
            $groupsParticipantId = [];
            ...
            {
                $orderGroupParticipant = OrderGroupParticipantWork::find()->where(['group_participant_id' => $delParticipant])->andWhere(['status' => $status])->one();
                if ($orderGroupParticipant !== null)
                    $orderGroupParticipant->delete();
            }
        }
    }
